// web/src/services/database.service.enhanced.ts
import { supabase } from '../lib/supabase';
import { logger } from '../lib/logger';
// @ts-ignore
import ResourceAwareScheduler from '@shared/algorithms/resource-aware/ResourceAwareScheduler';
// @ts-ignore
import { PatternDetector } from '@shared/algorithms/PatternDetector';
...
class EnhancedDatabaseService {
  ...
  private patternDetector = new PatternDetector(0.6);
      ...
      const result = await this.withRetry(
        async () => {
          ...
          if (excludeAnswered && profileId) {
            ...
            if (error) throw error;
            return { data: data || [], count: -1, error: null };
          }
  ...
  /**
   * ENHANCED: Get dynamic metrics for a user
   */
  ...
  /**
   * ENHANCED: Get patterns with caching and uncertainty quantification
   */
  async getPatterns(profileId: string, minConfidence: number = 0.6): Promise<any[]> {
    ...
    const result = await this.withRetry(
      async () => {
        ...
        // Augment with local uncertainty-aware patterns
        const localPatterns = this.patternDetector.getDetectedPatterns(profileId);

        const combined = [...(data || [])];

        // Merge or prioritize
        localPatterns.forEach(lp => {
          if (!combined.find(p => p.action === lp.action)) {
            combined.push({
              action: lp.action,
        ...
        const finalResult = combined.filter(p => p.confidence >= minConfidence);
  ...
  /**
   * ENHANCED: Resource-Aware Batch insert
   * Uses the ResourceAwareScheduler to optimize batch execution based on system resources
   */
  async batchInsert<T extends Record<string, any>>(
    ...
    // Advanced: Use ResourceAwareScheduler to manage batch execution
    const scheduler = new ResourceAwareScheduler(
      { cpu: 0.8, energy: 1000, memory: 512 * 1024 * 1024 }, // Budgets
      null,
      'greedy'
    );
    ...
    const tasks = batches.map((batch, index) => ({
      name: `batch_${table}_${index}`,
      operations: batch.length * 10, // Estimate 10 ops per record
      dataSize: JSON.stringify(batch).length,
      value: batch.length, // Value is proportional to batch size
      execute: async () => {
    ...
    // Use scheduler to optimize the schedule
    const { schedule, rejections } = await scheduler.optimizeSchedule(tasks);

    for (const item of schedule) {
      try {
        // Since optimizeSchedule already called execute(), we just collect results
        inserted += item.result;
      } catch (error) {
        errors.push({ task: item.task, error });
      }
    }

    for (const rejection of rejections) {
      errors.push({ task: rejection.task, reason: rejection.reason });
    }

    logger.info('Resource-aware batch insert completed', {
      table,
  ...
  /**
   * ENHANCED: Observe action for pattern detection
   */
  async observeAction(profileId: string, action: string, metadata?: any): Promise<void> {
    this.patternDetector.observe(profileId, action, metadata);
    logger.debug('Action observed', { profileId, action });
  }
