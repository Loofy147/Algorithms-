...
class SessionManager {
  ...
  static addActivityListeners() {
    ...
    const resetHandler = () => this.resetTimeout(30);
...
export const authService = {
  ...
  async updatePassword(newPassword: string): Promise<void> {
    const passwordValidation = this.validatePasswordEnhanced(newPassword);
    if (!passwordValidation.valid) {
      throw new AuthenticationError(
        passwordValidation.errors.join(', '),
        'WEAK_PASSWORD',
   ...
   * Initialize session management
   */
  initSessionManagement(timeoutMinutes: number = 30) {
  ...
  getSessionStatus(): { active: boolean; expiresAt: number | null } {
    ...
    const expiresAt = parseInt(lastActivity) + 30 * 60 * 1000;
    return {
